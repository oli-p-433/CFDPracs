...
int main(){
    ...
    double startTime = 0.0, endTime = 0.30;
    ...
    sim.fluxMethod = [&sim](bool direction) {
        sim.HLLCGodunov(direction); // Change this to sim.SLIC or sim.godunov as needed
    };
    ...
    for (size_t i=sim.ghosts(); i<uInit.size()-sim.ghosts();++i){
        ...
        for (size_t j=sim.ghosts(); j<uInit[0].size()-sim.ghosts();j++){
            ...
            */
            ...
            if (y < 0.25){
                uInit[i][j] = sim.eos[0]->primToConsv({vf,rho1*vf,rho2*(1-vf),0,0.3948,1.57});
            } else if (y < 0.4){
