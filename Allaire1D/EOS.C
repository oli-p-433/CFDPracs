...
double idealGas::calcSoundSpeed(const std::array<double,5>& arr){
...
double stiffenedGas::calcSoundSpeed(const std::array<double,5>& arr){
    ...
    double Y1 = arr[1]/(arr[1]+arr[2]);
    double Y2 = arr[2]/(arr[1]+arr[2]);
    double cs1 = gamma1*(arr[4]+p_inf1)/(arr[1]/arr[0]);
    double cs2 = gamma2*(arr[4]+p_inf2)/(arr[2]/(1-arr[0]));
    return std::sqrt((Y1*(1.0/(gamma1-1))*cs1+Y2*(1.0/(gamma2-1))*cs2)/eps);
}
...
std::array<double,2> stiffenedGas::get_pInf(){
    return {p_inf1,p_inf2};
}
...
std::array<double,5> stiffenedGas::primToConsv(const std::array<double,5>& arr)const{
    ...
    double stiff = arr[0]*(gamma1*p_inf1/(gamma1-1)) + (1-arr[0])*(gamma2*p_inf2/(gamma2-1));
    result[4] = arr[4]*((arr[0]/(gamma1-1))+(1-arr[0])/(gamma2-1)) + stiff + 0.5*(arr[1]+arr[2])*arr[3]*arr[3]; // allaire eq 31
...
std::array<double,5> stiffenedGas::consvToPrim(const std::array<double,5>& arr)const{
    ...
    double stiff = arr[0]*(gamma1*p_inf1/(gamma1-1)) + (1-arr[0])*(gamma2*p_inf2/(gamma2-1));
    result[4] = (arr[4] - stiff - 0.5*arr[3]*arr[3]/(arr[1]+arr[2]))/((arr[0]/(gamma1-1))+(1-arr[0])/(gamma2-1));
