...
std::array<double,3> riemann::exctRiemann(){ // states will be in primitive variable form by default
    //std::vector< std::array<double,3> > exctMesh;
    //exctMesh.resize(nCells);
    //double pStar = (1.0/(state1[0]+state2[0]))*(sqrt(state1[0]*gamma*state1[2])+sqrt(state2[0]*gamma*state2[2]));
...
riemann::riemann(double gL, double gR, std::array<double,3> stateL, std::array<double,3> stateR, double x_0,double x_1, double xDisc, double t, int N, double pIL, double pIR)
    : gammaL(gL), gammaR(gR),
        state1(stateL), state2(stateR),
        x0(x_0), x1(x_1),
